...
file_path <- "C:/Users/tpritch/Oregon/DEQ - Integrated Report - IR 2024/DraftList/Public Draft/Parameter Assessments/nonr_human/Non_R_assessments_2024_01_19.xlsx"
...
Other_categories <- other_AU_import |> 
  ...
  mutate(Delist_eligability = 0,
         ...
         recordID = paste0("2024-",odeqIRtools::unique_AU(AU_ID),"-", Pollu_ID, "-", wqstd_code,"-", period )) |> 
...
WS_GNIS_rollup <- WS_GNIS_import %>%
  ...
  mutate(Delist_eligability =  0) |> 
  mutate(IR_category_GNIS_24 = factor(IR_category_GNIS_24, levels=c('Unassessed', "3", "3B", "2", "5", "5C" ), ordered=TRUE)) |> 
...
AU_display <- bind_rows(AU_display_other, AU_display_ws) |> 
  ...
  mutate(Year_listed = case_when(final_AU_cat %in% c("5", '4A', "5C") & is.na(Year_listed) ~ '2024',
...
OceanAcidification_data <- read.xlsx(file_path,
                                  sheet = 'Ocean Acidification')

MarineDO_benchmark_data <- read.xlsx(file_path,
                                     sheet = 'Marine DO - Benchmark')


MarineDO_Background_data <- read.xlsx(file_path,
                                     sheet = 'Marine DO - Background')

AquaticTrash_data <- read.xlsx(file_path,
                                      sheet = 'Aquatic Trash')
  ...
  addWorksheet(wb, sheetName = "OceanAcidification_data", tabColour = 'paleturquoise2')
  addWorksheet(wb, sheetName = "MarineDO_benchmark_data", tabColour = 'paleturquoise2')
  addWorksheet(wb, sheetName = "MarineDO_Background_data", tabColour = 'paleturquoise2')
  addWorksheet(wb, sheetName = "AquaticTrash_data", tabColour = 'paleturquoise2')
  ...
  writeData(wb = wb, sheet = "OceanAcidification_data", x = OceanAcidification_data, headerStyle = header_st)
  writeData(wb = wb, sheet = "MarineDO_benchmark_data", x = MarineDO_benchmark_data, headerStyle = header_st)
  writeData(wb = wb, sheet = "MarineDO_Background_data", x = MarineDO_Background_data, headerStyle = header_st)
  writeData(wb = wb, sheet = "AquaticTrash_data", x = AquaticTrash_data, headerStyle = header_st)
